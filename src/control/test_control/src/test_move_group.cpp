...
bool copy_move_group_parameters(const rclcpp::Node::SharedPtr& node)
{
  ...
  if (!found_robot_description || !found_robot_description_semantic)
  {
    RCLCPP_ERROR(node->get_logger(),
                 ...
                 "Make sure start_lab_bench_moveit.launch.py is running.",
                 source_node.c_str());
    return false;
  }
...
public:
  ...
  {
    // 规划组 / TCP 名称参数化（默认适配 lab_bench 双臂）
    arm_a_group_name_ = node_->declare_parameter<std::string>("arm_a_group", "arm_A");
    arm_b_group_name_ = node_->declare_parameter<std::string>("arm_b_group", "arm_B");
    arm_both_group_name_ = node_->declare_parameter<std::string>("arm_both_group", "arm_BOTH");
    arm_a_tcp_ = node_->declare_parameter<std::string>("arm_a_tcp", "arm_A__tcp");
    arm_b_tcp_ = node_->declare_parameter<std::string>("arm_b_tcp", "arm_B__tcp");

    arm_a_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(node_, arm_a_group_name_);
    arm_b_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(node_, arm_b_group_name_);
    arm_both_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(node_, arm_both_group_name_);

    configure_group(arm_a_group_);
    configure_group(arm_b_group_);
    configure_group(arm_both_group_);

    RCLCPP_INFO(node_->get_logger(), "test_move_group initialized");
    RCLCPP_INFO(node_->get_logger(), "groups: %s / %s / %s, tcp: %s / %s",
                arm_a_group_name_.c_str(), arm_b_group_name_.c_str(), arm_both_group_name_.c_str(),
                arm_a_tcp_.c_str(), arm_b_tcp_.c_str());
  }

  void run()
  {
    RCLCPP_INFO(node_->get_logger(), "=== Starting MoveGroup planning/execution tests ===");
    bool arm_a_ok = test_single_arm(arm_a_group_, arm_a_group_name_, arm_a_tcp_);
    bool arm_b_ok = test_single_arm(arm_b_group_, arm_b_group_name_, arm_b_tcp_);
...
private:
    ...
    const std::string& target_name)
  {
    ...
    const bool a_set = arm_both_group_->setPoseTarget(arm_a_pose, arm_a_tcp_);
    const bool b_set = arm_both_group_->setPoseTarget(arm_b_pose, arm_b_tcp_);
    if (!a_set || !b_set)
    {
    ...
    RCLCPP_INFO(node_->get_logger(), "arm_BOTH %s target %s: %s", target_name.c_str(), arm_a_tcp_.c_str(),
                format_pose(arm_a_pose).c_str());
    RCLCPP_INFO(node_->get_logger(), "arm_BOTH %s target %s: %s", target_name.c_str(), arm_b_tcp_.c_str(),
  ...
  bool test_both_arm()
  {
    ...
    const auto initial_a = arm_both_group_->getCurrentPose(arm_a_tcp_);
    const auto initial_b = arm_both_group_->getCurrentPose(arm_b_tcp_);
    RCLCPP_INFO(node_->get_logger(), "arm_BOTH current pose %s: %s", arm_a_tcp_.c_str(), format_pose(initial_a.pose).c_str());
    RCLCPP_INFO(node_->get_logger(), "arm_BOTH current pose %s: %s", arm_b_tcp_.c_str(), format_pose(initial_b.pose).c_str());
  ...
  std::string arm_a_group_name_;
  std::string arm_b_group_name_;
  std::string arm_both_group_name_;
  std::string arm_a_tcp_;
  std::string arm_b_tcp_;
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("test_move_group", node_options);
