...
| 可执行节点 | 源文件 | 作用 |
|---------|--------|------|
| `test_ros2_control` | `src/test_ros2_control.cpp` | 直接向 ros2_control 控制器发送关节轨迹（action / topic / forward position），验证底层控制器 |
| `test_move_group` | `src/test_move_group.cpp` | 通过 MoveIt MoveGroup 接口做规划+执行，测试单臂与双臂规划组 |
| `test_move_servo` | `src/test_move_servo.cpp` | 向 MoveIt Servo 发送连续正弦位姿目标，测试实时伺服跟踪 |
| `railcar_sine_servo` | `src/railcar_sine_servo.cpp` | 读取小车正弦运动位置，并映射到 arm_A TCP 的 y 轴目标 |
| `test_mujoco_control` | `src/test_mujoco_control.cpp` | 向 forward_position_controller 与夹爪循环发送预设动作，用于 Mujoco 仿真 |
...
ros2 run test_control test_ros2_control

# 仅 arm_A，放宽到位容差
ros2 run test_control test_ros2_control --ros-args -p arm_target:=A -p goal_position_tolerance_rad:=0.02
```
...
```bash
# 先启动底层控制器与 MoveIt
ros2 launch launch_controller start_traditional_controllers.launch.py launch_rviz:=false
ros2 launch lab_bench_moveit_config start_lab_bench_moveit.launch.py launch_rviz:=true

# 再运行规划测试
ros2 run test_control test_move_group
```
...
ros2 launch launch_controller start_traditional_controllers.launch.py \
  launch_rviz:=false initial_ur_controller:=forward_position_controller
# 可组合组件方式会让 Servo 的状态监视回调在初始化期间得到执行；双臂本机测试推荐使用
ros2 launch lab_bench_moveit_config start_lab_bench_servo.launch.py launch_as_component:=true
...
ros2 run test_control test_move_servo --ros-args \
...
```bash
ros2 run test_control railcar_sine_servo

# 可按现场网络配置覆盖参数
ros2 run test_control railcar_sine_servo --ros-args \
...
`launch/test_control.launch.py` 提供一个示例启动文件，默认只启动 `test_move_group`，其余节点以注释形式列出，按需取消注释：

```bash
ros2 launch test_control test_control.launch.py
```
